...
class User_m extends SB_Model
{
    ...
    function login($data){
	    ...
	    if($user){
			...
			if($user['password']==$password){
				$fans = count($this->follow_m->get_fans_list($user['uid'],100000000000000000));
				$this->session->set_userdata(array ('uid' => $user['uid'], 'username' => $user['username'], 'group_type' => $user['group_type'], 'gid' => $user['gid'], 'avatar' => $user['avatar'], 'group_name' => $user['group_name'], 'is_active' => $user['is_active'], 'favorites' => $user['favorites'], 'follows' => $user['follows'],  'fans' => $fans, 'notices' => $user['notices'], 'messages_unread' => $user['messages_unread'], 'credit' => $user['credit'], 'lastpost' => $user['lastpost']));
				return TRUE;
			} else {
				return FALSE;
			}
	...
	{
		$query = $this->db->select('a.*,b.*')->from('users a')->join('user_groups b','b.group_type=a.group_type','LEFT')->where('a.email',$email)->get();
		return $query->row_array();
	}
